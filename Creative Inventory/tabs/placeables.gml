#define DrawPlaceablesTab(_menu)
    ...
    for (var i = 0; i < array_length(global.itemStructures); i++) {
        
        var _item = global.itemStructures[i];
...
#define StepPlaceablesTab
    ...
    for (var i = 0; i < array_length(global.itemStructures); i++) {
        ...
        if (mouse_get_raw_x() > global.CreativeMenuPos[0] + 5 + 50 * _xx and mouse_get_raw_x() < global.CreativeMenuPos[0] + 5 + 50 * _xx + 40 and mouse_get_raw_y() > global.CreativeMenuPos[1] + 55 + 50 * _yy - global.scroll and mouse_get_raw_y() < global.CreativeMenuPos[1] + 55 + 50 * _yy + 40 - global.scroll) {
            ...
            if (mouse_check_button_pressed(mb_right)) RemoveItem(_item, min(global.itemIncrements[global.itemIncrement][1], ItemCount(_item, true)));
