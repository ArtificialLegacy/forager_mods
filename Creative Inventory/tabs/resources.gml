#define DrawResourceTab(_menu)

    var _spawns = surface_create(600, 550);
    
    surface_set_target(_spawns);
    
    draw_clear_alpha(c_black, 0);
    
    for (var i = 0; i < array_length(global.spawns); i++) {
        ...
        draw_sprite_stretched(global.spawns[i][1], global.spawns[i][2], 9 + 50 * _xx, 9 + 50 * _yy, 32, 32);
    ...
    draw_surface(_spawns, 0, 50);

    surface_reset_target();
    
    surface_free(_spawns);
    
#define StepResourcesTab
    ...
    if (global.structureSelect == -1) {
        ...
        if (mouse_check_button_pressed(mb_right)) {
            ...
            var _inst = instance_create_depth(_xx, _yy, -_yy, global.spawns[global.structureSelect][0]);
            _inst.sprite_index = global.spawns[global.structureSelect][1];
            _inst.image_index = global.spawns[global.structureSelect][2];
