...
for (var i = 0; i < 9; i++) {
    ...
    with (_heart) {
        ...
        image_angle = (_dir + 90) - 40 + 10 * i;
        direction = _dir - 10 + 10 * i;
        speed = 3;
        
    }
    InstanceAssignMethod(_heart, "step", ScriptWrap(CelestialBowHeartStep), true);
    
}

#define CelestialBowHeartStep

    if (place_meeting(x, y, parEnemy)) {
        
        var _inst = instance_place(x, y, parEnemy);
        
        _inst.hp -= 50 + objPlayer.dmg;
