...
InstanceAssignMethod(_hitbox, "step", ScriptWrap(CelestialPickaxeHitboxStep), true);
...
for (var i = 0; i < 3; i++) {
    ...
    with (_heart) {
        ...
        image_angle = (_dir + 90) - 10 + 10 * i;
        direction = _dir - 10 + 10 * i;
        speed = 3;
        
    }
    InstanceAssignMethod(_heart, "step", ScriptWrap(CelestialPickaxeHeartStep), true);
    
}

#define CelestialPickaxeHitboxStep
    ...
    if (place_meeting(x, y, parEnemy)) {
        ...
        if (place_meeting(x, y, parNatural)) {
            ...
            if (instance_exists(_inst)) {
                ...
                _inst.hp -= 15 + (objPlayer.dmg * objPlayer.wrath);
                var _ex = instance_create_depth(x, y, 0, objExplosion);
                _ex.dmgPlayer = false;
...
#define CelestialPickaxeHeartStep
